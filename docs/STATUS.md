...
**Phase 1(모바일) 8개 태스크 중 2개 완료.** 이 프로젝트에서 가장 불확실했던 부분 — 앱이 종료된 상태에서 푸시 알림이 도착하는지 — 를 실기기에서 검증했다. 남은 작업은 평범한 UI 구현이다.

| Task | 내용 | 상태 |
|---|---|---|
| 1 | 프로젝트 생성, EAS 등록, Firebase, Development Build | ✅ 완료 |
| 2 | 푸시 권한·채널·토큰 발급, 실기기 수신 검증 | ✅ 완료 |
| 3 | 포그라운드 핸들러, 알림 탭 → 상세 라우팅 | ⬜ 다음 |
...
mobile/src/
  app/
    _layout.tsx     최소 Stack (Task 3에서 알림 핸들러 추가)
...
- `src/app/_layout.tsx`에 `setNotificationHandler` 추가 (포그라운드에서도 알림 표시)
- `useLastNotificationResponse`로 알림 탭 감지 → `/build/<id>`로 이동
- `src/app/build/[id].tsx` 임시 화면 생성

검증할 위험 두 가지:

1. **콜드 스타트 라우팅** — 앱이 종료된 상태에서 알림을 탭했을 때 목록이 아니라 상세 화면이 바로 열려야 한다
2. **`#` 문자** — `buildId`가 `my-service#42` 형태라 URL 경로에서 깨질 수 있다. 깨지면 쿼리 파라미터 방식으로 전환한다 (계획서에 대안 기재)
...
- Task 4의 `tailwind.config.js` `content`는 `./src/**`를 가리켜야 한다. 틀리면 클래스가 조용히 무시된다
